...
def register_user(request):
    email = request.POST['email']# reading email from register.html page
    password = request.POST['password']# reading password from register.html page
    username = request.POST['username']# reading user name from register.html page
    if User.objects.filter(username=username).exists():# check username is present or not
        return render(request, 'register.html', {'user_available': True}) # if it is present it will redirect to same page
    elif User.objects.filter(email=email).exists():# check email is present or not
        return render(request, 'register.html', {'email_available': True}) # if it is present it will redirect to same page
    else:
        user = User.objects.create_user(email=email, password=password, username=username) # store the data in user table
        user.save()
        return render(request, 'login.html', {'data': ''})
...
def view_expenses(request):
    all_expense = Expenses.objects.all()
...
def del_family_mem(request,id):
    family_mem = FamilyMembers.objects.get(id=id)
    family_mem.delete()
    all_updated_data = FamilyMembers.objects.all()
